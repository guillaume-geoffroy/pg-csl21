...
Partial metric spaces were  introduced in the early nineties  as a variant of metric spaces in which self-distances can be non-zero. Such spaces have attracted much attention in program semantics \cite{bkmp:partial-metrics, doi:10.1111/j.1749-6632.1994.tb44144.x, Samet:2013aa}, due to the fact that their topology is compatible with standard constructions from both domain theory (since it is $T_{0}$) and usual metric topology (\textit{e.g.} Cauchy sequences, completeness, Banach-fixed point theorem) \cite{bkmp:partial-metrics, doi:10.1111/j.1749-6632.1994.tb44144.x}.
...
In this paper we will only be concerned with partial metrics taking values over a \emph{commutative integral} quantale, of which we recall the definition below.
...
We recall now the definition of a generalised partial metric space:
...
For all metric spaces $(X,d)$, $(X, ([0,\infty], +, \geq), d)$ is a generalised partial metric space: notice that the triangular inequality in this definition is the opposite of the usual one, which corresponds to the fact that the quantale ordering on $\mathbb{R}$ is the dual of the usual ordering. 
Moreover, any GPMS $(X,Q,d)$ induces a (generalized) metric $d^{*}$ by letting 
