...
%
\caption{\small Our new metric is a partial metric: in the example above it can be seen that $d(f,h) \leq d(f,g)+d(g,h)- d(g,g)$ (with all distances computed in the interval $[x-\varepsilon, x+\varepsilon]$).}% and 
%
%
%minimum $\delta$ such that for all $y\in [x-\varepsilon, x+\varepsilon]$, both $g(y)\in [f(x)-\delta, f(x)+\delta]$ and $f(y)\in[g(x)-\delta, g(x)+\delta]$ hold. $\delta$ is thus $\max\{\delta_{1},\delta_{2}\}$ in the image above.}
\label{fig:graph2bis}
