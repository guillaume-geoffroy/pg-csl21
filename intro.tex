...
%  is that this notion is highly \emph{contextual}. 
 ...
 computationally intensive function is replaced by its Taylor's expansion {around some given point}, or in {approximate computing} techniques like \emph{loop perforation} \cite{loopperf}, in which a compiler can be asked to skip a certain number of iterations of a loop inside a program.% provided this won't exceed some error bound.
...
%
%  any diameter space yields a \emph{generalized partial metric} over its exact values.
%
%based on the definition, on each type, of a \emph{generalized partial metric} lifting the standard semantics of $\mathsf{Real}$ to higher-order types.
%
...
%
%$d_{\mathbb R}$ can be recovered from the diameter by $d_{\mathbb R}(x,y)=\mathsf{diam}([x,y])$, $\diam_{A}$
%
%
%
%Compared to other existing higher-order frameworks, the main novelty of these models is that any diameter space yields a \emph{generalized partial metric} over its exact values.
% % (which in turn leads to strengthen the usual triangular inequality as $d(x,y) + d(z,z)\leq d(x,z)+d(z,y)$).
%
%
%In any diameter space $(|A|, \intervals{A}, \distances{A}, \diam_{A})$, 
%the distance between two exact values $x,y\in |A|$ can be computed by measuring, through $\diam_{A}$, the size of the smallest approximate value which contains both. In other words, we obtain 
% a {generalized partial metric} $d_{A}:|A|\times |A|\to \distances{A}$ on $|A|$ by letting $d_{A}(x,y)=\diam_{A}(x\vee y)$. 
%In this way, we recover the standard metric on real numbers 
%lift the standard metric of 


%
%which
% generalize some properties of the diameter function of the standard metrics on real numbers, 
% yield a 
% 
% . In particular, just like $d_{\mathbb R}$ can be recovered from the diameter by $d_{\mathbb R}(x,y)=\mathsf{diam}([x,y])$,
...
In our model, while the distance between two real numbers is measured with the standard metrics (and thus lives in the quantale $[0,\infty]$), the distance between  two programs $f,g:\mathsf{Real}\to \mathsf{Real}$ lives in the functional quantale of monotone maps from \emph{approximate values} of $\mathsf{Real}$ (\emph{i.e.} closed intervals) to positive reals. This function associates a closed interval $a$ with the diameter of the smallest interval containing \emph{both} $f(a)$ and $g(a)$. 
Note that, with this definition, 
 the distance of $f$ \emph{from itself}, which needs not be (constantly) 0,  provides a measure of the sensitivity of $f$, since it associates each interval $a$ with the size of the interval $f(a)$ spanned by $f$ on $a$. %For instance, the self-distance of $\lambda x.\sin(x)$ associates any interval $a$ with a value $\leq 2$ (the diameter of $\sin(a)$).
