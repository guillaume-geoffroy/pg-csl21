...
\begin{example} We can take $\mathbb{C} = \operatorname{Set}$ (with morphisms ordered by equality), and $\dsp(\operatorname{Set})$ contains an object $\mathsf{Real}_{\operatorname{Set}}$ that represents the real numbers with their standard metric and compact intervals (plus $\emptyset$ and $\mathbb{R}$) as approximate values. Namely, $\bs{\mathsf{Real}_{\operatorname{Set}}} = \mathbb{R}$, $\intervals{\mathsf{Real}_{\operatorname{Set}}} = \{ \text{the compact intervals}, \emptyset, \mathbb{R} \}$, $\distances{\mathsf{Real}_{\operatorname{Set}}} = [0,\infty]$ and $\diam_{\mathsf{Real}_{\operatorname{Set}}}(I) = \operatorname{length}(I)$.

In this case, $\bs{\exp(\mathsf{Real}_{\operatorname{Set}}, \mathsf{Real}_{\operatorname{Set}})}$ is the set of all functions from $\mathbb{R}$ to $\mathbb{R}$, so $d_{\mathsf{Real}_{\operatorname{Set}}}$ defines a partial metric on all such functions.
\end{example}
